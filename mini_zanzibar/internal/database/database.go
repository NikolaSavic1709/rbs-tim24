...
func RedisNew() RedisService {
	...
	fullAddress := fmt.Sprintf("%s:%s", redis_address, redis_port)

	rdb := redis.NewClient(&redis.Options{
		Addr:     fullAddress,
		Password: redis_password,
		DB:       redis_database,
		// Note: It's important to add this for a secure connection. Most cloud redisServices that offer Redis should already have this configured in their redisServices.
	...
	s := &redisService{db: rdb, ctx: context.Background()}
...
func (s *redisService) Get(key string) ([]byte, error) {
	//ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second) // Default is now 5s
	//defer cancel()
	value, err := s.db.Get(s.ctx, key).Bytes()
	if err != nil {
		return nil, fmt.Errorf("failed to get key %v: %v", key, err)
	}
	return value, nil
}
